...
public class SubjectServiceImpl implements SubjectService {
    ...
    public List<SubjectIndexDocument> getSubjects(String languageCode) {
        return subjectRepository.findAllIncludingCachedUrlsAndTranslations().stream()
                .map(subject -> new SubjectIndexDocument(subject, languageCode))
                .collect(Collectors.toList());
    }

    @Override
    @MetadataQuery
    public List<SubjectIndexDocument> getSubjects(
        ...
        return publicIds.stream()
                ...
                .map(subjectRepository::findByPublicId)
                .filter(Objects::nonNull)
                .map(subject -> new SubjectIndexDocument(subject, languageCode))
                .collect(Collectors.toList());
    }
