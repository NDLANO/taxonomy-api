...
public class MetadataApiServiceImpl implements MetadataApiService {
    ...
    public MetadataDto getMetadataByPublicId(URI publicId) {
        ...
        try {
            final var returnedEntity = restTemplate
                    .getForEntity(getServiceUrl() + "/v1/taxonomy_entities/" + publicId, MetadataApiEntity.class)
                    .getBody();
    ...
    private List<MetadataDto> doBulkRead(String uri) {
        try {
            ...
            return Arrays.stream(returnedEntities).map(MetadataDto::new).collect(Collectors.toList());
        } catch (RestClientException exception) {
            throw new ServiceUnavailableException(exception);
        }
    }
    ...
    private List<MetadataDto> doBulkRead(Collection<URI> publicIds) {
        ...
        final var publicIdCommaSeparatedList = String.join(",",
                publicIds.stream().map(URI::toString).collect(Collectors.toSet()));
    ...
    private List<MetadataDto> doBulkRead(String key, String value) {
        ...
        var uriBuilder = UriComponentsBuilder.fromUriString(getServiceUrl() + "/v1/taxonomy_entities/")
                .queryParam("key", keyParam);

        if (valParam != null)
            uriBuilder = uriBuilder.queryParam("value", valParam);
    ...
    private void doEntitiesPut(Set<MetadataApiEntity> requestObjects) {
        ...
        while (true) {
            try {
                ...
                logger.warning("Error bulk updating metadata: " + exception.getMessage() + " Will retry " + retryTtl
                        + " more times");
    ...
    private List<MetadataDto> doBulkUpdate(Collection<URI> publicIds, MetadataDto metadataDto) {
        ...
        final var requestObjects = publicIds.stream().map(publicId -> {
        ...
        }).map(MetadataApiEntity::new).collect(Collectors.toSet());
    ...
    private Set<MetadataDto> doBulkActionAndReturnDtos(Collection<URI> publicIds,
            Function<Collection<URI>, List<MetadataDto>> action) {
        // Splits the collection of IDs into lists of maximum 100 entries each
        final var counter = new AtomicInteger(0);
        final var chunks = publicIds.stream()
                .collect(Collectors.groupingBy(iterator -> counter.getAndIncrement() / 100)).values();

        // Do action and return on each of the 100 entry chunks
        final var metadataFutureEntries = chunks.stream().map(list -> executor.submit(() -> action.apply(list)))
                .collect(Collectors.toSet());

        // Converts the list of futures into flat list of DTOs to return
        return metadataFutureEntries.stream().map(futureSet -> {
            try {
