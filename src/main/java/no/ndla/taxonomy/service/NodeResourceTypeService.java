...
@Service
public class NodeResourceTypeService {
    private final TopicRepository topicRepository;
    private final ResourceTypeRepository resourceTypeRepository;
    private final TopicResourceTypeRepository topicResourceTypeRepository;

    public NodeResourceTypeService(TopicRepository topicRepository, ResourceTypeRepository resourceTypeRepository, TopicResourceTypeRepository topicResourceTypeRepository) {
    ...
    @Transactional
    public List<TopicResourceType> getNodeResourceTypes(URI nodeId) {
        if (nodeId == null) {
            throw new InvalidArgumentServiceException(new NullPointerException("Node ID is null"));
        }
        Topic topic = topicRepository.findByPublicId(nodeId);
        if (topic == null) {
            throw new NotFoundServiceException("Node not found " + nodeId.toString());
        }
        return topicResourceTypeRepository.findAllByTopic(topic);
    }

    public URI addTopicResourceType(URI nodeId, URI resourceTypeId) {
        if (nodeId == null || resourceTypeId == null) {
            throw new InvalidArgumentServiceException(new NullPointerException("Node or resource type ID is null"));
        }
        Topic topic = topicRepository.findByPublicId(nodeId);
        if (topic == null) {
            throw new NotFoundServiceException("Node not found " + nodeId.toString());
    ...
    public void deleteNodeResourceType(URI connectionId) {
