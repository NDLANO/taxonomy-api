...
public class ResourceWithPathsIndexDocument extends ResourceIndexDocument {
    ...
    private void setPathsFromResource(Resource resource) {
        resource.getPrimaryResolvedPath()
                .map(ResolvedPath::getPath)
                .ifPresent(this::setPath);

        this.paths = resource.getResolvedPaths().stream()
                .map(ResolvedPath::getPath)
                .collect(Collectors.toList());

        // From original code, override default primary path
        this.setPath(null);
        for (final var resolvedPath : resource.getResolvedPaths()) {
            if (resolvedPath.isPrimary() && (this.getPath().isEmpty() || !this.getPath().get().startsWith("/topic"))) {
                this.setPath(resolvedPath.getPath());
            }
        }
    }
}
