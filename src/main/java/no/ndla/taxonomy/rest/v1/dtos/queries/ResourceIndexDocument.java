...
public class ResourceIndexDocument {
    ...
    public ResourceIndexDocument(Resource resource, String languageCode) {
        ...
        this.path = resource.getPrimaryResolvedPath().map(ResolvedPath::getPath).orElse(null);
        this.paths = resource.getResolvedPaths().stream().map(ResolvedPath::getPath).collect(Collectors.toSet());
    }
}
