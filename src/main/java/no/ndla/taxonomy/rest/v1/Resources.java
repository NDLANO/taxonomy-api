...
public class Resources extends CrudController<Resource> {
    ...
    public List<ResourceIndexDocument> index(
        ...
        return repository.findAllIncludingResolvedPathsAndTranslations()
                ...
                .map(resource -> {
                    ...
                    resource.getPrimaryResolvedPath().map(ResolvedPath::getPath).ifPresent(resourceIndexDocument::setPath);
    ...
    public ResourceIndexDocument get(
        ...
        final var resource = repository.findFirstByPublicIdIncludingResolvedPathsAndTranslations(id)
    ...
    public ResourceFullIndexDocument getResourceFull(
        ...
        final var resource = repository.findFirstByPublicIdIncludingResolvedPathsAndTranslations(id).orElseThrow(() -> new NotFoundHttpRequestException("Resource not found"));
        ...
        r.paths.addAll(resource.getResolvedPaths().stream().map(ResolvedPath::getPath).collect(Collectors.toList()));
        return r;
    }
