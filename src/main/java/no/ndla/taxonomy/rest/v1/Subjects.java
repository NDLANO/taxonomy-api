...
public class Subjects extends CrudControllerWithMetadata<Node> {
    ...
    public Subjects(TreeSorter treeSorter,
    ...
    public List<EntityWithPathChildDTO> getChildren(
            ...
            @ApiParam(value = "Select by relevance. If not specified, all nodes will be returned.")
                    URI relevance
    ) {
        ...
        filteredConnections.stream()
                .map(nodeConnection -> createChildDTO(subject, nodeConnection, language))
    ...
    private boolean searchForRelevance(NodeConnection connection, URI relevancePublicId, Collection<NodeConnection> children) {
        ...
        if (node != null) {
            ...
            children.stream()
                    .filter(st -> st.getParent().isPresent())
