...
public class Subjects extends CrudControllerWithMetadata<Subject> {
    ...
    public void put(
            ...
            @ApiParam(
                            name = "subject",
                            value = "The updated subject. Fields not included will be set to null.")
                    @RequestBody
    ...
    public ResponseEntity<Void> post(
            @ApiParam(name = "subject", value = "The new subject") @RequestBody
                    SubjectCommand command) {
    ...
    @ApiOperation(
            value = "Gets all topics associated with a subject",
            notes =
                    "This resource is read-only. To update the relationship between subjects and topics, use the resource /subject-topics.")
    @InjectMetadata
    public List<SubTopicIndexDocument> getTopics(
            ...
            @RequestParam(value = "filter", required = false, defaultValue = "")
                    @ApiParam(
                            value =
                                    "Select by filter id(s). If not specified, all topics will be returned."
        ...
        final var relevanceArgument =
                relevance == null || relevance.toString().equals("") ? null : relevance;

        final var subjectTopics =
                subjectTopicRepository.findAllBySubjectAndTopicId(subject, topicIds);
        final var topicSubtopics =
        ...
        final var filteredTopicSubtopics =
        ...
        return topicTreeSorter.sortList(returnList).stream()
                .distinct()
                .collect(Collectors.toList());
    }

    private SubTopicIndexDocument createSubTopicIndexDocument(
            Subject subject, DomainEntity connection, String language) {
        return new SubTopicIndexDocument(subject, connection, language);
    }

    private boolean hasFilterAndRelevanceOrJustFilterIfRelevanceIsNotSet(
            Topic topic, Collection<URI> filterPublicId, URI relevancePublicId) {
        return false;
    }

    private boolean searchForFilterOrRelevance(
            Object connection,
            Collection<URI> filterPublicId,
        ...
        if (connection instanceof TopicSubtopic) {
            ...
            topicSubtopic
                    ...
                    .ifPresent(
                            subtopic -> {
                                if (hasFilterAndRelevanceOrJustFilterIfRelevanceIsNotSet(
                                        subtopic, filterPublicId, relevancePublicId)) {
                    ...
                    .forEach(
                            topicSubtopicI -> {
                                if (topicSubtopicI
                                        ...
                                        .get()
                                        .getId()
                                        .equals(topicSubtopic.getSubtopic().get().getId())) {
                                    if (searchForFilterOrRelevance(
                                            topicSubtopicI,
                                            filterPublicId,
                                            relevancePublicId,
                                            topicSubtopics)) {
            ...
            if (topic != null) {
                if (hasFilterAndRelevanceOrJustFilterIfRelevanceIsNotSet(
                        topic, filterPublicId, relevancePublicId)) {
                    foundFilter.set(true);
                }
    ...
    @ApiOperation(
            value =
                    "Gets all resources for a subject. Searches recursively in all topics belonging to this subject."
    ...
    public List<ResourceWithTopicConnectionDTO> getResourcesForSubject(
        ...
        final Set<URI> resourceTypeIdSet =
                resourceTypeIds != null ? Set.of(resourceTypeIds) : Set.of();
        ...
        if (filterIdSet.isEmpty()) {
            return resourceService.getResourcesBySubjectId(
                    subjectId, resourceTypeIdSet, relevanceArgument, language);
        } else {
