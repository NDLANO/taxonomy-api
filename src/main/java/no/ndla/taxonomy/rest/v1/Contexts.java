...
public class Contexts {
    ...
    public List<ContextIndexDocument> get(
        ...
        final var subjects = subjectRepository.findAllIncludingResolvedPathsAndTranslations();
        final var topics = topicRepository.findAllByContextIncludingResolvedPathsAndTranslations(true);
        ...
        contextDocuments.addAll(subjects.stream()
                .map(subject -> new ContextIndexDocument(
                        ...
                        subject.getPrimaryResolvedPath().map(ResolvedPath::getPath).orElse(null)))
        ...
        contextDocuments.addAll(topics.stream()
                .map(topic -> new ContextIndexDocument(
                        ...
                        topic.getPrimaryResolvedPath().map(ResolvedPath::getPath).orElse(null)))
                .collect(Collectors.toList()));
