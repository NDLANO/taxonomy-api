...
public class SetSinglePrimaryConnectionOnResourcesSqlChange implements CustomSqlChange {
    ...
    private void setRandomPrimary(int resourceId) throws CustomChangeException {
        try (var query = connection.prepareStatement("SELECT id FROM topic_resource WHERE resource_id = " + resourceId + " LIMIT 1")) {
            ...
            try (var updateQuery = connection.prepareStatement("UPDATE topic_resource SET is_primary = TRUE WHERE id = " + connectionIdToUpdate)) {
    ...
    private void keepRandomPrimary(int resourceId) throws CustomChangeException {
        ...
        try (var query = connection.prepareStatement("SELECT id FROM topic_resource WHERE resource_id = " + resourceId + " AND is_primary = TRUE LIMIT 1")) {
            final var result = query.executeQuery();
            ...
            try (var updateQuery = connection.prepareStatement("UPDATE topic_resource SET is_primary = FALSE WHERE id != " + connectionIdToKeep + " AND resource_id = " + resourceId)) {
                updateQuery.executeUpdate();
            }
        } catch (SQLException | DatabaseException e) {
            throw new CustomChangeException(e);
