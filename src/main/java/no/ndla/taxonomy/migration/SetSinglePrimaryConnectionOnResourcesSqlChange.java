...
public class SetSinglePrimaryConnectionOnResourcesSqlChange implements CustomSqlChange {
    ...
    private void setRandomPrimary(int resourceId) throws CustomChangeException {
        try (var query =
                connection.prepareStatement(
                        "SELECT id FROM topic_resource WHERE resource_id = "
                                + resourceId
                                + " LIMIT 1")) {
            ...
            if (!result.next()) {
                throw new CustomChangeException(
                        "No results found when trying to set random primary connection");
            }
            ...
            try (var updateQuery =
                    connection.prepareStatement(
                            "UPDATE topic_resource SET is_primary = TRUE WHERE id = "
    ...
    private void keepRandomPrimary(int resourceId) throws CustomChangeException {
        ...
        try (var query =
            ...
            try (var updateQuery =
                    connection.prepareStatement(
                            "UPDATE topic_resource SET is_primary = FALSE WHERE id != "
                                    + connectionIdToKeep
                                    + " AND resource_id = "
                                    + resourceId)) {
                updateQuery.executeUpdate();
    ...
    public SqlStatement[] generateStatements(Database database) throws CustomChangeException {
        ...
        try {
            try (var resourcesWithOtherThanOnePrimaryConnectionQuery =
                ...
                final var resourceListQuery =
                        resourcesWithOtherThanOnePrimaryConnectionQuery.executeQuery();
