...
public interface ResourceTypeRepository extends TaxonomyRepository<ResourceType> {
    @Query("SELECT DISTINCT rt" + "   FROM ResourceType rt" + "   LEFT JOIN FETCH rt.subtypes"
    ...
    @Query("SELECT DISTINCT rt" + "   FROM ResourceType rt" + "   LEFT JOIN FETCH rt.resourceTypeTranslations"
            + "   WHERE rt.publicId = :publicId")
    Optional<ResourceType> findFirstByPublicIdIncludingTranslations(URI publicId);

    @Query("SELECT DISTINCT rt" + "   FROM ResourceType rt" + "   JOIN rt.parent prt"
            + "   LEFT JOIN FETCH rt.resourceTypeTranslations" + "   LEFT JOIN FETCH rt.subtypes"
