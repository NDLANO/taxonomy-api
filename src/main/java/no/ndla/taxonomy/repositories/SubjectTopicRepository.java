...
public interface SubjectTopicRepository extends TaxonomyRepository<SubjectTopic> {
    ...
    List<SubjectTopic> findAllByTopicPublicIdIncludingSubjectAndTopicAndCachedUrls(
            URI topicPublicId);
    ...
    default List<SubjectTopic> findAllBySubjectAndTopicId(
            Subject subject, Collection<Integer> topicId) {
        if (topicId.size() == 0) {
