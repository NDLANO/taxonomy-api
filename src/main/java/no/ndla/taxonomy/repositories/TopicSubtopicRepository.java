...
public interface TopicSubtopicRepository extends TaxonomyRepository<TopicSubtopic> {
    ...
    List<TopicSubtopic> findAllBySubtopicPublicIdIncludingTopicAndSubtopicAndResolvedPaths(URI subTopicPublicId);

    @Query("SELECT DISTINCT ts" +
            "   FROM TopicSubtopic ts" +
            "   INNER JOIN FETCH ts.topic t" +
            "   INNER JOIN FETCH ts.subtopic st" +
            "   LEFT OUTER JOIN FETCH st.resolvedPaths" +
            "   WHERE t.publicId = :topicPublicId")
    List<TopicSubtopic> findAllByTopicPublicIdIncludingTopicAndSubtopicAndResolvedPaths(URI topicPublicId);
}