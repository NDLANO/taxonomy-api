...
class MetadataUpdateServiceImplTest {
    ...
    void setUp(
        ...
        metadataUpdateService =
                new MetadataUpdateServiceImpl(
                        subjectRepository, topicRepository, resourceRepository, metadataApiService);
    }
    ...
    void updateMetadataRecursivelyByPublicId() {
        {
            ...
            final var expectedUpdated =
                    Set.of("urn:subject:mdup:1", "urn:topic:mdup:11", "urn:topic:mdup:12");
            ...
            doAnswer(
                            invocation -> {
                                ...
                                updatePublicIds.forEach(
                                        publicId -> {
                                            assertTrue(
                                                    expectedUpdated.contains(publicId.toString()));
                    ...
                    .when(metadataApiService)
                    .updateMetadataByPublicIds(anySet(), any(MetadataDto.class));

            final var result =
                    metadataUpdateService.updateMetadataRecursivelyByPublicId(
                            URI.create("urn:subject:mdup:1"), metadataDto, false);
            assertEquals(3, result.getUpdatedCount());
            assertEquals(3, result.getUpdated().size());
            assertTrue(
                    result.getUpdated().stream()
                            .map(URI::toString)
                            .collect(Collectors.toSet())
...

        {
            ...
            final var expectedUpdated =
                    Set.of("urn:subject:mdup:2", "urn:topic:mdup:21", "urn:topic:mdup:22");
            final var actuallyUpdated = new ArrayList<>();
            ...
            doAnswer(
                            invocation -> {
                                ...
                                assertTrue(
                                        metadataObject.getGrepCodes().size() == 2
                                                && metadataObject
                                                        .getGrepCodes()
            ...
            final var result =
                    metadataUpdateService.updateMetadataRecursivelyByPublicId(
                            URI.create("urn:subject:mdup:2"), metadataDto, false);
            assertEquals(3, result.getUpdatedCount());
            assertEquals(3, result.getUpdated().size());
            assertTrue(
                    result.getUpdated().stream()
...

        {
            ...
            final var expectedUpdated =
                    Set.of(
                            ...
                            "urn:topic:mdup:21",
                            "urn:topic:mdup:22",
                            "urn:resource:mdup:211",
                            "urn:resource:mdup:212",
                            "urn:resource:mdup:221",
                            "urn:resource:mdup:222");
            final var actuallyUpdated = new ArrayList<>();
            ...
            final var result =
                    metadataUpdateService.updateMetadataRecursivelyByPublicId(
                            URI.create("urn:subject:mdup:2"), metadataDto, true);
...

        {
            ...
            final var expectedUpdated =
                    Set.of("urn:topic:mdup:21", "urn:resource:mdup:211", "urn:resource:mdup:212");
            final var actuallyUpdated = new ArrayList<>();
            ...
            final var result =
                    metadataUpdateService.updateMetadataRecursivelyByPublicId(
                            URI.create("urn:topic:mdup:21"), metadataDto, true);
            assertEquals(3, result.getUpdatedCount());
            assertEquals(3, result.getUpdated().size());
            assertTrue(
                    result.getUpdated().stream()
...

        {
            ...
            final var result =
                    metadataUpdateService.updateMetadataRecursivelyByPublicId(
                            URI.create("urn:resource:mdup:212"), metadataDto, true);
