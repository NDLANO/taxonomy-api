...
public class UrlResolverTest extends RestTest {
    ...
    public void can_resolve_url_for_topic() throws Exception {
        ...
        builder.node(NodeType.SUBJECT, version, s -> s.isContext(true).publicId("urn:subject:1").child(NodeType.TOPIC,
                version, t -> t.publicId("urn:topic:1").name("the topic").contentUri("urn:article:1")));
    ...
    public void can_resolve_url_for_subtopic() throws Exception {
        Version version = versionService.getPublished().get();
        builder.node(NodeType.SUBJECT, version,
                s -> s.isContext(true).publicId("urn:subject:1").child(NodeType.TOPIC, version,
                        t -> t.publicId("urn:topic:1").child(NodeType.TOPIC, version,
                                st -> st.publicId("urn:topic:2").contentUri("urn:article:1"))));
    ...
    public void gets_404_on_wrong_path_to_resource() throws Exception {
        Version version = versionService.getPublished().get();
        builder.node(NodeType.SUBJECT, version, s -> s.isContext(true).publicId("urn:subject:1").child(NodeType.TOPIC,
                version, t -> t.publicId("urn:topic:1").resource(r -> r.publicId("urn:resource:1"))));
