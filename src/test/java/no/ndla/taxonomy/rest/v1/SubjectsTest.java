...
public class SubjectsTest extends RestTest {
    ...
    public void can_get_single_subject() throws Exception {
        builder.subject(
                s -> s.name("english").contentUri("urn:article:1").publicId("urn:subject:1"));
        ...
        assertTrue(
                subject.getMetadata().getGrepCodes().size() == 1
    ...
    public void can_get_all_subjects() throws Exception {
        ...
        SubjectIndexDocument[] subjects =
                testUtils.getObject(SubjectIndexDocument[].class, response);
    ...
    public void can_create_subject() throws Exception {
        final var createSubjectCommand =
                new SubjectCommand() {
                    {
                        name = "testsubject";
        ...
        MockHttpServletResponse response =
                testUtils.createResource("/v1/subjects", createSubjectCommand);
    ...
    public void can_delete_subject() throws Exception {
        URI id =
                builder.subject(
                                s ->
                                        s.topic(t -> t.publicId("urn:topic:1"))
    ...
    public void can_get_topics() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource("/v1/subjects/" + subject.getPublicId() + "/topics");
        ...
        assertAnyTrue(
                topics,
                t -> "statics".equals(t.name) && "urn:article:1".equals(t.contentUri.toString()));
        assertAnyTrue(
                topics,
                t ->
                        "electricity".equals(t.name)
                                && "urn:article:2".equals(t.contentUri.toString()));
        assertAnyTrue(
                topics,
                t -> "optics".equals(t.name) && "urn:article:3".equals(t.contentUri.toString()));
    ...
    public void can_get_topics_recursively() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource("/v1/subjects/" + subjectid + "/topics?recursive=true");
        SubTopicIndexDocument[] topics =
                testUtils.getObject(SubTopicIndexDocument[].class, response);
        ...
        assertEquals(
                first(parent.getChildrenTopicSubtopics()).getPublicId(), topics[1].connectionId);

        Topic child = builder.topic("child");
        assertEquals(
                first(child.getChildrenTopicSubtopics()).getPublicId(), topics[2].connectionId);
    }
    ...
    public void recursive_topics_with_filter_are_ordered_relative_to_parent() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/subjects/urn:subject:1/topics?recursive=true&filter=urn:filter:1");
        SubTopicIndexDocument[] topics =
                testUtils.getObject(SubTopicIndexDocument[].class, response);
        ...
        MockHttpServletResponse response2 =
                testUtils.getResource(
                        "/v1/subjects/urn:subject:1/topics?recursive=true&filter=urn:filter:2");
        SubTopicIndexDocument[] topics2 =
                testUtils.getObject(SubTopicIndexDocument[].class, response2);
