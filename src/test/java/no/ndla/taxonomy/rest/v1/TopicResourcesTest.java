...
public class TopicResourcesTest extends RestTest {
    ...
    public void can_add_resource_to_topic() throws Exception {
        ...
        assertAnyTrue(
                calculus.getResources(), t -> "Introduction to integration".equals(t.getName()));
    ...
    public void can_get_resources() throws Exception {
        ...
        TopicResources.TopicResourceIndexDocument[] topicResources =
                testUtils.getObject(TopicResources.TopicResourceIndexDocument[].class, response);

        assertEquals(2, topicResources.length);
        assertAnyTrue(
                topicResources,
                t ->
                        electricity.getPublicId().equals(t.topicid)
                                && alternatingCurrent.getPublicId().equals(t.resourceId));
        assertAnyTrue(
                topicResources,
                t ->
                        calculus.getPublicId().equals(t.topicid)
    ...
    public void can_get_topic_resource() throws Exception {
        ...
        MockHttpServletResponse resource =
                testUtils.getResource("/v1/topic-resources/" + topicResource.getPublicId());
        TopicResources.TopicResourceIndexDocument topicResourceIndexDocument =
                testUtils.getObject(TopicResources.TopicResourceIndexDocument.class, resource);
    ...
    public void resource_can_only_have_one_primary_topic() throws Exception {
        ...
        builder.topic(t -> t.name("elementary maths").resource(graphs));

        Topic graphTheory = builder.topic(t -> t.name("graph theory"));
    ...
    public void can_order_resources() throws Exception {
        ...
        testUtils.updateResource(
                "/v1/topic-resources/" + geometryCircles,
                new TopicResources.UpdateTopicResourceCommand() {
                    {
                        primary = true;
                        id = geometryCircles;
        ...
        testUtils.updateResource(
                "/v1/topic-resources/" + geometrySquares,
                new TopicResources.UpdateTopicResourceCommand() {
                    {
                        primary = true;
                        id = geometrySquares;
        ...
        MockHttpServletResponse response =
                testUtils.getResource("/v1/topics/" + geometry.getPublicId() + "/resources");
        ResourceIndexDocument[] resources =
    ...
    public void update_child_resource_rank_modifies_other_contiguous_ranks() throws Exception {
        ...
        testUtils.updateResource(
                "/v1/topic-resources/" + updatedConnection.getPublicId().toString(),
        ...
        for (TopicResource topicResource : topicResources) {
            MockHttpServletResponse response =
                    testUtils.getResource(
                            "/v1/topic-resources/" + topicResource.getPublicId().toString());
    ...
    @Test
    public void update_child_resource_rank_higher_rank_does_not_modify_existing_connections()
            throws Exception {
        ...
        assertEquals(10, updatedConnection.getRank());
        testUtils.updateResource(
                "/v1/topic-resources/" + topicResources.get(9).getPublicId().toString(),
                new SubjectTopics.UpdateSubjectTopicCommand() {
                    {
                        primary = true;
                        rank = 99;
