...
public class TopicFiltersTest extends RestTest {
    ...
    public void can_list_all_topic_filters() throws Exception {
        ...
        TopicFilters.TopicFilterIndexDocument[] topicFilters =
                testUtils.getObject(TopicFilters.TopicFilterIndexDocument[].class, response);
    ...
    public void can_get_resources_belonging_to_a_filter_for_a_topic() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource("/v1/topics/urn:topic:1/resources?filter=urn:filter:1:1");
    ...
    public void can_get_resources_for_a_topic_belonging_to_a_filter_recursively() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/topics/urn:topic:a/resources?recursive=true&filter=urn:filter:1:1");
        ResourceIndexDocument[] result =
                testUtils.getObject(ResourceIndexDocument[].class, response);
                ...
                builder.resourceType(
                        rt ->
                                rt.name("Subject matter")
                                        .publicId("urn:resourcetype:subject-matter"));

        builder.topic(
                t ->
                        t.publicId("urn:topic:1")
                                .resource(r -> r.publicId("urn:resource:1").resourceType(type))
                                .resource(r -> r.publicId("urn:resource:2")));

        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/topics/urn:topic:1/resources?type="
                                + type.getPublicId()
                                + "&filter=urn:filter:1:1");
        ResourceIndexDocument[] result =
                testUtils.getObject(ResourceIndexDocument[].class, response);
    ...
    public void can_get_resources_for_a_topic_with_resource_type() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/topics/urn:topic:1/resources?type=" + type.getPublicId());
        ResourceIndexDocument[] result =
                testUtils.getObject(ResourceIndexDocument[].class, response);
    ...
    @Test
    public void can_get_recursive_resources_with_filter_and_resource_type_restrictions_for_a_topic()
            throws Exception {
        ResourceType type =
                builder.resourceType(
                        rt ->
        ...
        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/topics/urn:topic:a/resources?recursive=true&filter=urn:filter:1:1&type="
                                + type.getPublicId());
        ResourceIndexDocument[] result =
                testUtils.getObject(ResourceIndexDocument[].class, response);
    ...
    public void can_get_resources_by_relevance() throws Exception {
        ...
        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/topics/urn:topic:1/resources?relevance=" + core.getPublicId());
        ResourceIndexDocument[] result =
                testUtils.getObject(ResourceIndexDocument[].class, response);
    ...
    public void can_get_resources_by_relevance_recursively() throws Exception {
                                                                                ...
                                                                                .resource(
                                                                                        r ->
                                                                                                r.name(
                                                                                                                "resource aa")
                                                                                                        .contentUri(
                                                                                                                "urn:article:aa") /*.filter(vg1, core)*/)
                                                                                .subtopic(
                                                                                        st2 ->
                                                                                                st2.name(
                                                                                                                "aaa")
                                                                                                        .resource(
                                                                                                                r ->
                                                                                                                        r.name(
                                                                                                                                        "resource aaa")
                                                                                                                                .contentUri(
                                                                                                                                        "urn:article:aaa") /*.filter(vg1, core)*/))
                                                                                .subtopic(
                                                                                        st2 ->
                                                                                                st2.name(
                                                                                                                "aab")
                                                                                                        .resource(
                                                                                                                r ->
                                                                                                                        r.name(
                                                                                                                                        "resource aab")
                                                                                                                                .contentUri(
                                                                                                                                        "urn:article:aab") /*.filter(vg1, supplementary)*/)))));

        MockHttpServletResponse response =
                testUtils.getResource(
                        "/v1/topics/urn:topic:a/resources?recursive=true&relevance="
                                + core.getPublicId());
        ResourceIndexDocument[] result =
                testUtils.getObject(ResourceIndexDocument[].class, response);
        ...
        assertAnyTrue(
                result,
                r ->
                        "resource a".equals(r.name)
                                && "urn:article:a".equals(r.contentUri.toString()));
        assertAnyTrue(
                result,
                r ->
                        "resource aa".equals(r.name)
                                && "urn:article:aa".equals(r.contentUri.toString()));
        assertAnyTrue(
                result,
                r ->
                        "resource aaa".equals(r.name)
                                && "urn:article:aaa".equals(r.contentUri.toString()));
    }
}
