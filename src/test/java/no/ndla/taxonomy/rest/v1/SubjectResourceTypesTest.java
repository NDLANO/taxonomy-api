...
public class SubjectResourceTypesTest extends RestTest {
    ...
    public void can_have_several_resource_types_recursively() throws Exception {
        URI subjectId = builder
                .subject(s -> s.name("subject")
                        .topic(t -> t.name("topic").subtopic(st -> st.resource(r -> r.name("resource 1")
                                .resourceType(rt -> rt.name("lecture")).resourceType(rt -> rt.name("assignment"))))
    ...
    public void can_get_resources_for_a_subject_filtered_on_resource_type() throws Exception {
        ...
        URI subjectId = builder.subject(s -> s.name("subject")
                .topic(t -> t.name("a")
                        .subtopic(sub -> sub.name("subtopic")
        ...
        MockHttpServletResponse response = testUtils
                .getResource("/v1/subjects/" + subjectId + "/resources?type=" + lecture);
        ResourceIndexDocument[] result = testUtils.getObject(ResourceIndexDocument[].class, response);
